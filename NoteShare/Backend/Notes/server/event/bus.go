...
type DefaultBus struct {
	...
	notifyChan    chan *models.EventMessage
}
...
func NewDefaultBus() *DefaultBus {
	return &DefaultBus{
		...
		notifyChan:    make(chan *models.EventMessage, 0),
	}
...
func (d *DefaultBus) Subscribe(name string, eventType ...string) *Subscription {
	...
	subscription := &Subscription{
		Name:      name,
		EventChan: make(chan *models.EventMessage, 1),
...
func (d *DefaultBus) Notify(event *models.EventMessage) {
	d.notifyChan <- event
}
